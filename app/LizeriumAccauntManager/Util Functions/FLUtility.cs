 ...
 * Last Updated: 08 сентября 2026 06:53:06
 * Version: 1.0.157
 */
...
namespace Root
{
    class FLUtility
    {
        ...
        public static uint CreateID(string nickName)
        {
            ...
            byte[] tNickName = Encoding.Default.GetBytes(nickName.ToLowerInvariant());
        ...
        public static string GetLocation(IFLGameDataService gameData, FLDataFileService charFile)
        {
            ...
            if (charFile.SettingExists("Player", "pos"))
            {
                ...
                location += String.Format(" в космосе {0}, {1}, {2}", posX, posY, posZ);
            }
            else
            {
                location += " состыкован с " + gameData.GetItemDescByNickNameX(charFile.GetSetting("Player", "base").Str(0));
            }
            return location;
        }
        ...
        public static string GetShip(IFLGameDataService gameData, IFLDataFileService charFile, out Int64 shipArchType)
        {
            string nickNameOrHash = charFile.GetSetting("Player", "ship_archetype").Str(0);
            HashListItem shipItem = gameData.GetItemByNickName(nickNameOrHash);
            if (shipItem != null)
            {
        ...
        public static string EncodeUnicodeForRtf(string input)
        {
        ...
        public static DateTime GetTimeStamp(FLDataFileService charFile)
        {
        ...
        public static uint GetOnLineTime(FLDataFileService charfile)
        {
        ...
        /// <summary>
        /// Сканирование всех учетных записей в фоновом режиме.
        /// </summary>
        /// <param name="progress">Обновления хода работ публикуются здесь.</param>
        /// <param name="token">Токен отмены операции</param>
        /// <param name="log">Интерфейс журнала</param>
        /// <param name="gameData">Данные игры фрилансера</param>
        /// <returns>DataModel</returns>
        public static async Task<DataModel> CheckAccounts(IProgress<ProgressInfo> progress,
            CancellationToken token,
            ILogRecorder log, IFLGameDataService gameData)
        {
            FLHookSocketService flsk = new FLHookSocketService();

            // Пока мы это делаем, понизим приоритет процесса.
            System.Diagnostics.Process.GetCurrentProcess().PriorityClass =
                System.Diagnostics.ProcessPriorityClass.BelowNormal;

            DateTime updateTime = DateTime.Now.AddMilliseconds(100);

            DataModel tempDataStore = new DataModel();
            using (DataBaseService dataBaseService = new DataBaseService())
            {
                ...
                {
                    await Task.WhenAll(
                        dataBaseService.GetBanList(tempDataStore),
                        dataBaseService.GetCharacterList(tempDataStore));
                });

                log.AddLog("Load complete "
                    + tempDataStore.DataSetPlayerInfo.CharacterList.Count + " characters, "
                    + tempDataStore.DataSetPlayerInfo.BanList.Count + " bans");

                // Очистить явно поврежденные и старые файлы
                if (AppSettings.Default.setAutomaticCharClean)
                {
                    int totalFilesDeleted = 0;
                    progress.Report(new(0, "Cleaning accounts..."));
                    log.AddLog("Cleaning accounts...");
                    string[] accDirs = Directory.GetDirectories(AppSettings.Default.setAccountDir, "??-????????");
                    for (int i = 0; i < accDirs.Length && !token.IsCancellationRequested; i++)
                    {
                        if (updateTime < DateTime.Now)
                        {
                            updateTime = DateTime.Now.AddMilliseconds(1000);
                            progress.Report(new((i * 100) / accDirs.Length, ""));
                        }
                        string accDir = accDirs[i].Substring(AppSettings.Default.setAccountDir.Length + 1);
                        totalFilesDeleted += FLUtility.CleanCharAccount(log, gameData, accDir);
                    }
                    log.AddLog("Removed " + totalFilesDeleted + " files");
                }

                // Проверьте наличие новых/измененных файлов персонажей.
                int totalFilesUpdated = 0;
                progress.Report(new(0, "Checking for new or changed characters..."));
                ...
                for (int i = 0; i < accDirs1.Length && !token.IsCancellationRequested; i++)
                {
                    if (updateTime < DateTime.Now)
                    {
                        updateTime = DateTime.Now.AddMilliseconds(1000);
                        progress.Report(new((i * 100) / accDirs1.Length, ""));
                    }
                    string accDir = accDirs1[i].Substring(AppSettings.Default.setAccountDir.Length + 1);
                    totalFilesUpdated += FLUtility.ScanCharAccount(tempDataStore, dataBaseService, log, gameData, accDir);
                }
                log.AddLog("Updated " + totalFilesUpdated + " files");


                // Проверьте наличие удаленных файлов персонажей и учетных записей и удалите неактивных/неинтересных игроков.
                progress.Report(new(0, "Checking for deleted and inactive/uninterested characters..."));
                log.AddLog("Checking for deleted and inactive/uninterested characters...");
                int inactiveChars = 0;
                int uninterestedChars = 0;
                int len = tempDataStore.DataSetPlayerInfo.CharacterList.Count;
                for (int i = 0; i < len && !token.IsCancellationRequested; i++)
                {
                    CharacterItem charRecord = tempDataStore.DataSetPlayerInfo.CharacterList[i];
                    if (updateTime < DateTime.Now)
                    {
                        updateTime = DateTime.Now.AddMilliseconds(1000);
                        progress.Report(new((i * 100) / len, ""));
                    }
                    ...
                    if (!charRecord.IsDeleted && AppSettings.Default.setAutomaticCharWipe)
                    {
                        ...
                        {
                            // Удалить персонажа можно и через FLHook.
                            if (flsk.IsConnected())
                ...
                // Зафиксируйте все изменения в базе данных.
                log.AddLog("Saving database...");
                progress.Report(new(50, "Saving database..."));
                await dataBaseService.InsertOrUpdatePlayerInfoChanges(tempDataStore, log);
        ...
        public static int ScanCharAccount(DataModel dataSet, IDataBaseService dataBase, ILogRecorder log, IFLGameDataService gameData, string accDir)
        {
            int filesUpdated = 0;
            ...
            var banRecord = dataSet.DataSetPlayerInfo.FindBanByAccDir(accDir);
            if (banRecord != null && !banned)
            {
                dataBase.DeleteBanRecordFromDatabase(accDir);
                filesUpdated++;
            }
            else if (banRecord == null && banned)
            {
                var newBan = new BanItem
                {
                    AccDir = accDir,
                    AccID = GetAccountID(accDirPath),
                    BanReason = "UNKNOWN",
                    BanStart = File.GetCreationTime(accDirPath + Path.DirectorySeparatorChar + "banned"),
                    BanEnd = DateTime.Now.AddDays(1000).ToUniversalTime()
                };
                dataSet.DataSetPlayerInfo.BanList.Add(newBan);
                // Сохраняем новый бан в базе
                dataBase.InsertOrUpdateBanRecordToDatabase(newBan);
                filesUpdated++;
            }
            ...
            try
            {
                ...
                {
                    string loginID = Path.GetFileNameWithoutExtension(loginFilePath).Substring(6); // "login_" - 6 символов
                    DateTime accessTime = File.GetLastWriteTime(loginFilePath);

                    var loginRecord = new LoginIDItem
                    {
                        AccDir = accDir,
                        LoginID = loginID,
                    ...
                    dataSet.LoginIDList.Add(loginRecord);

                    // Сохраняем в базу (если нужно делать сразу, либо позже пакетно)
                    dataBase.InsertOrUpdateLoginIdRecord(loginRecord);
                }
            }
            catch (Exception ex)
            {
            ...
            foreach (string charFilePath in charFiles)
            {
                try
                {
                    ...
                    CharacterItem charRecord = dataSet.DataSetPlayerInfo.FindByCharPath(charPath);
                    if (charRecord != null)
                    {
                    ...
                    FLDataFileService cfp = new FLDataFileService(charFilePath, true);
                    ...
                    if (charRecord == null)
                    {
                        charRecord = new CharacterItem
                        {
                            ...
                            CharName = name,
                            CharPath = charFilePath.Substring(AppSettings.Default.setAccountDir.Length + 1),
                            AccID = GetAccountID(accDirPath),
                            AccDir = accDir,
                            Updated = DateTime.Now,
                            IsDeleted = false,
                            Location = GetLocation(gameData, cfp),
                            Money = cfp.GetSetting("Player", "money").UInt(0),
                            Rank = cfp.GetSetting("Player", "rank").UInt(0),
                            Ship = FLUtility.GetShip(gameData, cfp, out long shipHash),
                            OnLineSecs = GetOnLineTime(cfp),
                            LastOnLine = GetTimeStamp(cfp),
                        };
                        dataSet.DataSetPlayerInfo.CharacterList.Add(charRecord);
                        // Возможно, тут надо сразу сохранить в БД, или сохранить потом пакетно
                        dataBase.InsertOrUpdateCharacter(charRecord);
                    }
                    ...
                    else
                    {
                        ...
                        charRecord.Money = cfp.GetSetting("Player", "money").UInt(0);
                        charRecord.Rank = cfp.GetSetting("Player", "rank").UInt(0);
                        long shipHash = 0;
                        charRecord.Ship = FLUtility.GetShip(gameData, cfp, out shipHash);
                        charRecord.OnLineSecs = GetOnLineTime(cfp);
        ...
        public static int CheckForDeletedChars(DataSetPlayerInfo dataSetPlayerInfo, IDataBaseService dataBase, string accDir)
        {
            int filesUpdated = 0;
            // Поиск всех персонажей с данным accDir
            var charRecords = dataSetPlayerInfo.CharacterList.Where(c => c.AccDir == accDir).ToList();

            foreach (var charRecord in charRecords)
            {
                string charFilePath = Path.Combine(AppSettings.Default.setAccountDir, charRecord.CharPath);
        ...
        /// <returns></returns>
        private static int CleanCharAccount(ILogRecorder log, IFLGameDataService gameData, string accDir)
        {
            int filesDeleted = 0;
            string accDirPath = AppSettings.Default.setAccountDir + "\\" + accDir;
        ...
        private static bool CheckEquipment(DataModel dataSet, ILogRecorder log, IFLGameDataService gameData,
            IFLDataFileService charFile, bool admin, string setting, bool fixErrors)
        {
            ...
            if (UInt32.TryParse(charFile.GetSetting("Player", "ship_archetype").Str(0), out shipHash))
            {
                ...
                ShipInfoItem shipInfo = gameData.GetShipInfo(shipHash);
                HardPointItem[] hardPoints = gameData.GetHardPointListByShip(shipHash);
                if (shipInfo == null)
                {
                ...
                foreach (FLDataFileSetting set in charFile.GetSettings("Player", setting))
                {
                    try
                    {
                        uint equipHash = set.UInt(0);

                        HashListItem equipItem = gameData.GetItemByHash(equipHash);
                        if (equipItem == null)
                        {
                        ...
                        if (equipItem.ItemType == gameData.GAMEDATA_ENGINES)
                        {
                            if (foundEngine)
                            {
                        ...
                        else if (equipItem.ItemType == gameData.GAMEDATA_POWERGEN)
                        {
                            if (foundPowerGen)
                            {
                        ...
                        else if (equipItem.ItemType == gameData.GAMEDATA_SCANNERS)
                        {
                            if (foundScanner)
                            {
                        ...
                        else if (equipItem.ItemType == gameData.GAMEDATA_TRACTORS)
                        {
                            if (foundTractor)
                            {
                        ...
                        else if (equipItem.ItemType == gameData.GAMEDATA_SOUND)
                        {
                            if (foundSound)
                            {
                ...
                foreach (FLDataFileSetting set in charFile.GetSettings("player", setting))
                {
                    try
                    {
                        ...
                        HardPointItem hardPointInfo = Array.Find(hardPoints, delegate(HardPointItem value) 
                            { 
                                return hardpoint == value.HPName.ToLowerInvariant(); 
                            });
                        ...
                        EquipInfoItem equipItem = gameData.GetEquipInfo(equipHash);
                        if (equipItem != null && !hardPointInfo.MountableTypes.Contains(equipItem.MountableType))
                        {
                            if (!admin)
                            {
                                ...
                                if (fixErrors)
                                {
                                    if (setting == "base_equip" && equipItem.ItemType == gameData.GAMEDATA_GUNS)
                                        charFile.AddSettingNotUnique("Player", "cargo", new object[] { equipHash, 1, "", "", 0 });
                                    charFile.DeleteSetting(set);
                                }
                            }
                        ...
                        {
                            HashListItem defaultItem = gameData.GetItemByHash(hardPointInfo.DefaultItemHash);
                            if (defaultItem != null)
                            {
                                if (defaultItem.ItemType == gameData.GAMEDATA_LIGHTS
                                    || defaultItem.ItemType == gameData.GAMEDATA_FX)
                                {
        ...
        public static bool CheckCharFile(DataModel dataSet, ILogRecorder log, IFLGameDataService gameData, 
            IFLDataFileService charFile, bool admin, bool fixErrors)
        {
            bool foundErrors = false;

            // Check the hashcodes.
            foreach (FLDataFileSetting set in charFile.GetSettings("Player"))
            {
                try
                {
            ...
            foreach (FLDataFileSetting set in charFile.GetSettings("mPlayer"))
            {
                try
                {
        ...
        /// </summary>
        /// <returns>True if errors are detected</returns>
        public static string PrettyPrintCharFile(IFLGameDataService gameData, IFLDataFileService charFile)
        {
            ...
            foreach (FLDataFileSetting set in charFile.GetSettings("Player"))
            {
                ...
                try
                {
                        ...
                        || set.settingName == "com_costume")
                    {
                        ...
                        if (UInt32.TryParse(set.Str(0), out hash))
                        {                           
                            HashListItem item = gameData.GetItemByHash(hash);
                            if (item != null)
                        ...
                        {
                            HashListItem item = gameData.GetItemByNickName(set.Str(0));
                            if (item != null)
        ...
        public static string GetPlayerInfoText(string accDirPath, string charFilePath)
        {
            ...
            try
            {
                ...
                {
                    FLDataFileService data = new FLDataFileService(filePath, false);
                    string text = "";
                    for (int i = 1; i < 10; i++)
                    {
                        if (data.SettingExists("Info", i.ToString()))
                        {
                            FLDataFileSetting set = data.GetSetting("Info", i.ToString());
        ...
        public static string GetPlayerInfoAdminNote(string accDirPath, string charFilePath)
        {
            ...
            try
            {
                if (File.Exists(filePath))
                {
                    ...
                    {
                        FLDataFileSetting set = data.GetSetting("Info", "AdminNote");
                        if (set != null && set.NumValues() > 0)
        ...
        public static void SetPlayerInfoAdminNote(string accDirPath, string charFilePath, string text)
        {
            ...
            try
            {
                FLDataFileService data = new FLDataFileService(false);
                if (File.Exists(filePath))
                    data = new FLDataFileService(filePath, false);
