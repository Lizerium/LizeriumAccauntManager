...
{
    public partial class FLDataFileService : IFLDataFileService
    {
        /// <summary>
        ...
        /// </summary>
        public List<FLDataFileSection> Sections { get; set; } = new List<FLDataFileSection>();
        ...
        public FLDataFileService(bool multiFields)
        {
        ...
        /// <param name="multiFields">if true commas delimit fields</param>
        public FLDataFileService(string filePath, bool multiFields)
        {
            this.multiFields = multiFields;
        ...
        /// <param name="multiFields">if true commas delimit fields</param>
        public FLDataFileService(byte[] buf, string fileName, bool multiFields)
        {
        ...
        void LoadBinaryIni(byte[] buf, string fileName)
        {
            ...
            while (p < buf.Length && p < strTableOffset)
            {
                ...
                FLDataFileSection section = new FLDataFileSection();
                ...
                while (sectionNumEntries-- > 0)
                {
                    ...
                    FLDataFileSetting setting = new FLDataFileSetting(sectionName, settingName, desc, values);
        ...
        string BufToString(byte[] buf, int offset)
        {
            ...
            for (int i=offset; i<buf.Length && buf[i]!=0; i++, strLen++)
                ;
            return Encoding.Default.GetString(buf, offset, strLen);
        }

        void LoadTextIni(byte[] buf, string fileName)
        {
            FLDataFileSection sec = null;
                        ...
                        else
                        {
                            ...
                            if (strLine != "")
                            {
                                ...
                                if (multiFields)
                                {
                                    ...
                                    set = new FLDataFileSetting(sec.sectionName, settingName, desc, settingValues);
                                }
                                else
                                {
        ...
        {
            foreach (FLDataFileSection sec in Sections)
        ...
        /// <param name="sectionName">Section name.</param>
        /// <param name="settingName">Key name.</param>
        public FLDataFileSetting GetSetting(string sectionName, string settingName)
        {
            foreach (FLDataFileSection sec in Sections)
                if (sec.sectionName.ToLowerInvariant() == sectionName.ToLowerInvariant())
        ...
        public bool AddSetting(string sectionName, string settingName, object[] settingValues)
        {
            foreach (FLDataFileSection sec in Sections)
            {
                if (sec.sectionName.ToLowerInvariant() == sectionName.ToLowerInvariant())
                {
                    ...
                    foreach (FLDataFileSetting set in sec.settings)
                    {
                    ...
                    sec.settings.Add(new FLDataFileSetting(sec.sectionName, settingName, "unknown", settingValues));
                    return true;
                }
            }

            // The entry section wasn't found, add a new section.
            FLDataFileSection ns = new FLDataFileSection();
            ns.sectionName = sectionName;
            ns.settings.Add(new FLDataFileSetting(sectionName, settingName, "unknown", settingValues));
            Sections.Add(ns);
            return true;
        }
        ...
        public bool DeleteSetting(FLDataFileSetting setting)
        {
            foreach (FLDataFileSection sec in Sections)
            {
                foreach (FLDataFileSetting set in sec.settings)
        ...
        /// <returns></returns>
        public List<FLDataFileSetting> GetSettings(string sectionName, string settingName)
        {
            List<FLDataFileSetting> result = new List<FLDataFileSetting>();
            foreach (FLDataFileSection sec in Sections)
            {
                if (sec.sectionName.ToLowerInvariant() == sectionName.ToLowerInvariant())
                {
        ...
        /// <param name="sectionName">The section name.</param>
        /// <returns></returns>
        public List<FLDataFileSetting> GetSettings(string sectionName)
        {
            List<FLDataFileSetting> result = new List<FLDataFileSetting>();
            foreach (FLDataFileSection sec in Sections)
            {
        ...
        public void SaveSettings(string filePath, bool encrypt)
        {
            ...
            foreach (FLDataFileSection sec in Sections)
            {
                ...
                foreach (FLDataFileSetting setting in sec.settings)
                {
